...
import com.example.doandidong.Interface.llmageClickListenner;
import com.example.doandidong.R;
import com.example.doandidong.model.EventBus.TinhTongEvent;
        ...
        holder.setListenner(new llmageClickListenner() {
            ...
            public void onImageClick(View view, int pos, int giatri) {
                if (giatri == 1) { // Trừ
                    ...
                    if (gioHangList.get(pos).getSoluong() < gioHangList.get(pos).getSltonkho()) { // Giới hạn tối đa 10
                        int soluongmoi = gioHangList.get(pos).getSoluong() + 1;
                        gioHangList.get(pos).setSoluong(soluongmoi);
                        holder.item_giohang_soluong.setText(soluongmoi + " ");
                        long gia = soluongmoi * gioHangList.get(pos).getGiasp();
